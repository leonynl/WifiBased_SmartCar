...
const char* ssid = "AAAAAA_SMART";
const char* password = "adminadmin";

String tcp_str = "";      //接收到的信息
bool result; // WIFI mode setting result

IPAddress softLocal(192,168,77,1);
IPAddress softGateway(192,168,77,1);
IPAddress softSubnet(255,255,255,0);

// 创建一个服务器实例
// 指定要监听的端口作为参数
WiFiServer server(77); //创建server
...
{
   uint8_t i;
   if(result && server.hasClient()){                 //判断当前是否有客户端
...
//          continue;
    ...
    //检查客户端的数据
    for(i = 0; i < MAX_SRV_CLIENTS; i++){
      if(result && serverClients[i] && serverClients[i].connected()){
